...
varsig2_grid = c( 0.01, 0.1, 1,  10, 50 )*(var(as.vector(data), na.rm = T)^2)#*seq(0.001,2,length.out = N_varsd)
...
rownames(RI_mat_Bin) <- rownames(RI_mat_VI) <- rownames(L1_mat) <- rownames(Kest_mat) <- rownames(Kvar_mat) <- paste0("corr=", correction_grid)
colnames(RI_mat_Bin) <- colnames(RI_mat_VI) <- colnames(L1_mat) <- colnames(Kest_mat) <- colnames(Kvar_mat) <- paste0("*", round(seq(0.001,2,length.out = N_varsd), digits = 3) )
...
rownames(RI_mat_Bin) <- rownames(RI_mat_VI) <- rownames(L1_mat) <- rownames(Kest_mat) <- rownames(Kvar_mat) <- paste0("corr=", correction_grid)
colnames(RI_mat_Bin) <- colnames(RI_mat_VI) <- colnames(L1_mat) <- colnames(Kest_mat) <- colnames(Kvar_mat) <- paste0("varmu=", varmu_grid)
...
rownames(RI_mat_Bin) <- rownames(RI_mat_VI) <- rownames(L1_mat) <- rownames(Kest_mat) <- rownames(Kvar_mat) <- paste0("*", round(seq(0.001,2,length.out = N_varsd), digits = 3) )
colnames(RI_mat_Bin) <- colnames(RI_mat_VI) <- colnames(L1_mat) <- colnames(Kest_mat) <- colnames(Kvar_mat) <- paste0("varmu=", varmu_grid)
...
In questo esperimento, fisso gli iperparametri di $P_0$ e mi concentro solo su $\Lambda$. Infatti dall'altro caso studio ho visto che il ruolo di $\Lambda$ è fondamentale nel definire il numero di cluster a priori.

Negli esperimenti di prima avevo $\Lambda \sim \text{Gamma}(10,2)$, quindi $\text{E}[\Lambda] = 5$, $\text{Var}(\Lambda) = 2.5$.

Come prima prova, non lascio $\Lambda$ aleatorio ma lo fisso
...
for(jj in 1:N_Lambda){
    ...
    option = set_options_marginal(
                 ...
                 "Adapt_MH_hyp1"= 0.7,"Adapt_MH_hyp2"= 0.234, 
                 "sp_mala_U" = 0.01, "sp_mala_gamma"=0.01,
                 "gamma0" = 3,"Lambda0" = Lambda_grid[jj], 
                 "alpha_gamma"=a_gamma, "beta_gamma"=b_gamma, 
                 "alpha_lambda"=a_lambda, "beta_lambda"=b_lambda,
                 "init_mean_cluster" = NULL,#unlist(mean_data_per_cluster), 
                 "init_var_cluster" = NULL,#unlist(var_data_per_cluster),
                 "UpdateU" = T, "UpdateGamma" = T, "UpdateTau" = T, "UpdateLambda" = F, 
                 "partition" = real_partition#seq(1,sum(n_j))
            )
...
names(Lambda_mean) <- names(Lambda_mean) <- paste0("Lambda=", Lambda_grid)
...
rownames(gamma_means) <- paste0("Lambda=", Lambda_grid)
...
names(RI_Bin) <- names(RI_VI) <- names(L1) <- names(Kest) <- names(Kest_var) <- names(CoClustering_error) <- paste0("Lambda=", Lambda_grid)
...
Rispetto alla sezione precedente, non lascio $\Lambda$ fisso ma aleatorio. Parametrizzo $\Lambda$ come media e varianza. Metto la media pari ai valori fissati prima e varianza 1
...
mean_Lambda_grid = seq(1,20,by = 2)
var_lambda = 1
