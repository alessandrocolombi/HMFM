...
for(jj in 1:N_varmu){
    ...
    res = list("P0_hyparam"= c("mu0"=mu0,"k0"=k0,"nu0"=nu0,"sigma0"=sigma0),
               ...
               "Kstats_lv1" = c("MCmedian" = median(Kest_lv1), "MCvar" = var(Kest_lv1) ),
               "Kstats_lv2" = c("MCmedian" = median(Kest_lv2), "MCvar" = var(Kest_lv2) ),
...
Kest_VI = unlist(lapply(FinalResults, FUN = function(x){
  x$Kstats[4]
}))

Kest_Bin = unlist(lapply(FinalResults, FUN = function(x){
  x$Kstats[3]
}))
...
MC_Kest_lv1 = unlist(lapply(FinalResults, FUN = function(x){
  x$Kstats_lv1[1]
}))

Kest_var_lv1 = unlist(lapply(FinalResults, FUN = function(x){
  x$Kstats_lv1[2]
}))

MC_Kest_lv2 = unlist(lapply(FinalResults, FUN = function(x){
  x$Kstats_lv2[1]
}))

Kest_var_lv2 = unlist(lapply(FinalResults, FUN = function(x){
  x$Kstats_lv2[2]
}))

names(RI_Bin) <- names(RI_VI) <- names(L1) <- names(Kest_VI) <- names(Kest_Bin) <- names(MC_Kest_lv1)<- names(MC_Kest_lv2)<- names(Kest_var_lv1) <- names(Kest_var_lv2) <- names(Kest_var) <- names(MC_Kest) <- names(CoClustering_error) <- nomi_varmu_grid
...
names(RI_Bin) <- names(RI_VI) <- names(L1) <- names(Kest_VI) <- names(Kest_Bin) <- names(MC_Kest_lv1)<- names(MC_Kest_lv2)<- names(Kest_var_lv1) <- names(Kest_var_lv2) <- names(Kest_var) <- names(MC_Kest) <- names(CoClustering_error) <- nomi_varsig2_grid
...
names(RI_Bin) <- names(RI_VI) <- names(L1) <- names(Kest_VI) <- names(Kest_Bin) <- names(MC_Kest_lv1)<- names(MC_Kest_lv2)<- names(Kest_var_lv1) <- names(Kest_var_lv2) <- names(Kest_var) <- names(MC_Kest) <- names(CoClustering_error) <- nomi_correction_grid
...
names(RI_Bin) <- names(RI_VI) <- names(L1) <- names(Kest_VI) <- names(Kest_Bin) <- names(MC_Kest_lv1)<- names(MC_Kest_lv2)<- names(Kest_var_lv1) <- names(Kest_var_lv2) <- names(Kest_var) <- names(MC_Kest) <- names(CoClustering_error) <- nomi_meanL_grid
...
names(RI_Bin) <- names(RI_VI) <- names(L1) <- names(Kest_VI) <- names(Kest_Bin) <- names(MC_Kest_lv1)<- names(MC_Kest_lv2)<- names(Kest_var_lv1) <- names(Kest_var_lv2) <- names(Kest_var) <- names(MC_Kest) <- names(CoClustering_error) <- nomi_varL_grid
...
names(RI_Bin) <- names(RI_VI) <- names(L1) <- names(Kest_VI) <- names(Kest_Bin) <- names(MC_Kest_lv1)<- names(MC_Kest_lv2)<- names(Kest_var_lv1) <- names(Kest_var_lv2) <- names(Kest_var) <- names(MC_Kest) <- names(CoClustering_error) <- nomi_bgamma_grid
...
Kest_VI_mat = apply(FinalResults, c(1,2), FUN = function(x){
  x[[1]]$Kstats[4]
})

Kest_Bin_mat = apply(FinalResults, c(1,2), FUN = function(x){
  x[[1]]$Kstats[3]
})
...
MC_Kest_lv1_mat = apply(FinalResults, c(1,2), FUN = function(x){
  x[[1]]$Kstats_lv1[1]
})

Kvar_lv1_mat = apply(FinalResults, c(1,2), FUN = function(x){
  x[[1]]$Kstats_lv1[2]
})

MC_Kest_lv2_mat = apply(FinalResults, c(1,2), FUN = function(x){
  x[[1]]$Kstats_lv2[1]
})

Kvar_lv2_mat = apply(FinalResults, c(1,2), FUN = function(x){
  x[[1]]$Kstats_lv2[2]
})


rownames(RI_mat_Bin) <- rownames(RI_mat_VI) <- rownames(L1_mat) <- rownames(Kest_VI_mat) <- rownames(Kest_Bin_mat) <- rownames(MC_Kest_lv1_mat) <- rownames(MC_Kest_lv2_mat) <- rownames(Kvar_lv1_mat) <- rownames(Kvar_lv2_mat) <- rownames(Kvar_mat) <- rownames(MC_Kest_mat) <- nomi_meanL_grid
colnames(RI_mat_Bin) <- colnames(RI_mat_VI) <- colnames(L1_mat) <- colnames(Kest_VI_mat) <- colnames(Kest_Bin_mat) <- colnames(MC_Kest_lv1_mat) <- colnames(MC_Kest_lv2_mat) <- colnames(Kvar_lv1_mat) <- colnames(Kvar_lv2_mat) <- colnames(Kvar_mat) <- colnames(MC_Kest_mat) <- nomi_bgamma_grid
...
rownames(RI_mat_Bin) <- rownames(RI_mat_VI) <- rownames(L1_mat) <- rownames(Kest_VI_mat) <- rownames(Kest_Bin_mat) <- rownames(MC_Kest_lv1_mat) <- rownames(MC_Kest_lv2_mat) <- rownames(Kvar_lv1_mat) <- rownames(Kvar_lv2_mat) <- rownames(Kvar_mat) <- rownames(MC_Kest_mat) <- nomi_correction_grid
colnames(RI_mat_Bin) <- colnames(RI_mat_VI) <- colnames(L1_mat) <- colnames(Kest_VI_mat) <- colnames(Kest_Bin_mat) <- colnames(MC_Kest_lv1_mat) <- colnames(MC_Kest_lv2_mat) <- colnames(Kvar_lv1_mat) <- colnames(Kvar_lv2_mat) <- colnames(Kvar_mat) <- colnames(MC_Kest_mat) <- nomi_varsig2_grid
...
rownames(RI_mat_Bin) <- rownames(RI_mat_VI) <- rownames(L1_mat) <- rownames(Kest_VI_mat) <- rownames(Kest_Bin_mat) <- rownames(MC_Kest_lv1_mat) <- rownames(MC_Kest_lv2_mat) <- rownames(Kvar_lv1_mat) <- rownames(Kvar_lv2_mat) <- rownames(Kvar_mat) <- rownames(MC_Kest_mat) <- nomi_correction_grid
colnames(RI_mat_Bin) <- colnames(RI_mat_VI) <- colnames(L1_mat) <- colnames(Kest_VI_mat) <- colnames(Kest_Bin_mat) <- colnames(MC_Kest_lv1_mat) <- colnames(MC_Kest_lv2_mat) <- colnames(Kvar_lv1_mat) <- colnames(Kvar_lv2_mat) <- colnames(Kvar_mat) <- colnames(MC_Kest_mat) <- nomi_varmu_grid
...
rownames(RI_mat_Bin) <- rownames(RI_mat_VI) <- rownames(L1_mat) <- rownames(Kest_VI_mat) <- rownames(Kest_Bin_mat) <- rownames(MC_Kest_lv1_mat) <- rownames(MC_Kest_lv2_mat) <- rownames(Kvar_lv1_mat) <- rownames(Kvar_lv2_mat) <- rownames(Kvar_mat) <- rownames(MC_Kest_mat) <- nomi_varsig2_grid
colnames(RI_mat_Bin) <- colnames(RI_mat_VI) <- colnames(L1_mat) <- colnames(Kest_VI_mat) <- colnames(Kest_Bin_mat) <- colnames(MC_Kest_lv1_mat) <- colnames(MC_Kest_lv2_mat) <- colnames(Kvar_lv1_mat) <- colnames(Kvar_lv2_mat) <- colnames(Kvar_mat) <- colnames(MC_Kest_mat) <- nomi_varmu_grid
...
lists = vector("list", length = N_varmu*N_gamma)

FinalResults = matrix(lists, nrow = N_varmu, ncol = N_gamma, byrow = T)
```
...
for(ii in 1:N_varmu){
  for(jj in 1:N_gamma){
    ...
    cat("\n Sim Number ",nsim,"\n")
    # Define hyperparameters
    P0_hyparam = empirical_bayes_normalinvgamma(data = data, varmu = varmu_grid[ii], 
                                                varsig2 = vardata^2, 
                                                correction = 3
                                                )
    mu0 = P0_hyparam$mu0
...
rownames(nu0s) <- rownames(k0s) <- rownames(sigma0s) <- nomi_varmu_grid
colnames(nu0s) <- colnames(k0s) <- colnames(sigma0s) <- nomi_bgamma_grid
...
rownames(PriorVar) <- nomi_varmu_grid
colnames(PriorVar) <- nomi_bgamma_grid
...
rownames(Lambda_mean_mat) <- rownames(Lambda_var_mat) <- nomi_varmu_grid
colnames(Lambda_mean_mat) <- colnames(Lambda_var_mat) <- nomi_bgamma_grid
...
rownames(RI_mat_Bin) <- rownames(RI_mat_VI) <- rownames(L1_mat) <- rownames(Kest_VI_mat) <- rownames(Kest_Bin_mat) <- rownames(MC_Kest_lv1_mat) <- rownames(MC_Kest_lv2_mat) <- rownames(Kvar_lv1_mat) <- rownames(Kvar_lv2_mat) <- rownames(Kvar_mat) <- rownames(MC_Kest_mat) <- nomi_varmu_grid
colnames(RI_mat_Bin) <- colnames(RI_mat_VI) <- colnames(L1_mat) <- colnames(Kest_VI_mat) <- colnames(Kest_Bin_mat) <- colnames(MC_Kest_lv1_mat) <- colnames(MC_Kest_lv2_mat) <- colnames(Kvar_lv1_mat) <- colnames(Kvar_lv2_mat) <- colnames(Kvar_mat) <- colnames(MC_Kest_mat) <- nomi_bgamma_grid
