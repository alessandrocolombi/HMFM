...
# 3 groups at a time vs all groups together -------------------------------
data_level1 = as.matrix(data_level1)
GDFMM1 = GDFMM_sampler(data_level1, niter, burnin, thin, seed = 123, option = option)

part_matrix <- GDFMM1$Partition
sim_matrix <- psm(part_matrix)
binder_dahl <- dlso(part_matrix, loss = 'binder', estimate = sim_matrix)

est_part1 = as.vector(binder_dahl)


option_fixed1 <- list("Mstar0" = 100, "Lambda0"= 2, "mu0"=0, "nu0"=10, "sigma0"= 1,
                     "Adapt_MH_hyp1"=0.7,"Adapt_MH_hyp2"=0.234, "Adapt_MH_power_lim"=10, "Adapt_MH_var0"=1,
                     "k0"= 1/14, "alpha_gamma"=1,
                     "beta_gamma"=1, "alpha_lambda"=1, "beta_lambda"=1, "partition" = est_part1)

GDFMM_fixed1 = GDFMM_sampler(data_level1, niter, burnin, 3, seed = 123, FixPartition = T, option = option_fixed1)

GDFMM2 = GDFMM_sampler(data_level2, niter, burnin, thin, seed = 123, option = option)
part_matrix <- GDFMM2$Partition
sim_matrix <- psm(part_matrix)
binder_dahl <- dlso(part_matrix, loss = 'binder', estimate = sim_matrix)

est_part2 = as.vector(binder_dahl)

option_fixed2 <- list("Mstar0" = 100, "Lambda0"= 2, "mu0"=0, "nu0"=10, "sigma0"= 0.5,
                      "Adapt_MH_hyp1"=0.7,"Adapt_MH_hyp2"=0.234, "Adapt_MH_power_lim"=10, "Adapt_MH_var0" = 1,
                      "k0"= 1/10, "alpha_gamma"=1,
                      "beta_gamma"=1, "alpha_lambda"=1, "beta_lambda"=1, "partition" = est_part2)

GDFMM_fixed2 = GDFMM_sampler(data_level2, niter, burnin, 3, seed = 123,
                             FixPartition = T, option = option_fixed2)

data_level3 = t(as.matrix(data_level3))
GDFMM3 = GDFMM_sampler(data_level3, niter, burnin, thin, seed = 123, option = option)

part_matrix <- GDFMM3$Partition
sim_matrix <- psm(part_matrix)
binder_dahl <- dlso(part_matrix, loss = 'binder', estimate = sim_matrix)

est_part3 = as.vector(binder_dahl)

option_fixed3 <- list("Mstar0" = 100, "Lambda0"= 2, "mu0"=0, "nu0"=10, "sigma0"= 1,
                      "Adapt_MH_hyp1"=0.7,"Adapt_MH_hyp2"=0.234, "Adapt_MH_power_lim"=10, "Adapt_MH_var0"=1,
                      "k0"= 1/10, "alpha_gamma"=1,
                      "beta_gamma"=1, "alpha_lambda"=1, "beta_lambda"=1, "partition" = est_part3)

GDFMM_fixed3 = GDFMM_sampler(data_level3, niter, burnin, 3, seed = 123,
                            FixPartition = T, option = option_fixed3)

k_est = max(est_part3) + 1
w_only3 = rowMeans(GDFMM_fixed3$w_jk[[1]])

# compute mean of mu from GS
mu_list = GDFMM_fixed3$mu
mu_GS3 = c()
for(k in 1:k_est){
  mu_GS3 = c(mu_GS3, mean(mu_list[[k]]))
}

# # compute mean of sigma from GS
sigma_list = GDFMM_fixed3$sigma
sigma_GS3 = c()
for(k in 1:k_est){
  sigma_GS3 = c(sigma_GS3, sqrt( mean(sigma_list[[k]]) ) )
}

x11()
plot(grid, dmix(grid, c(10/21, 1/21, 10/21), c(-3, 3, 0), c(1/2, 1/2, 1/2)),
     main = "Density comparison for group 3", ylab = "Group 3",
     type = "l", lty = 2, col = "black", lwd = 2)
lines(grid, dmix(grid, w_only3, mu_GS3, sigma_GS3), lty = 2, col = "red", lwd = 2)
lines(grid, mix_3, lwd = 2, col = "blue")
legend("topright", legend = c("real", "1 group at a time", "3 groups considered"),
       col = c("black", "red", "blue"), lty = c(2, 2, 1), lwd = 2)
...
thin = 2
seed = 2561996
option <- list("Mstar0" = 2, "Lambda0" = 2, "mu0" = 0, "nu0"= 10, "sigma0"= 1,
               "k0"= 1/10, "Adapt_MH_hyp1" = 0.7, "Adapt_MH_hyp2" = 0.234,
...
thin = 2
seed = 2561996
option <- list("Mstar0" = 4, "Lambda0" = 2, "mu0" = 0, "nu0"= 10, "sigma0"= 1,
               "k0"= 1/10, "Adapt_MH_hyp1" = 0.7, "Adapt_MH_hyp2" = 0.234,
               "Adapt_MH_power_lim" = 10, "Adapt_MH_var0"=1,
               "alpha_gamma" = 1, "beta_gamma" = 1,
               "alpha_lambda" = 1, "beta_lambda" = 1)
