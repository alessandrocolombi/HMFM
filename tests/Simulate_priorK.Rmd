...
Non va bene tutto quello che ho fatto. Ho tolto i dati solo dalla partizione ma poi le altre quantità sono comunque estratte dalle full conditional. Devo estrarre tutto dalla prior, giusto?
Posso risolvere solo se tengo $\gamma$ e $\Lambda$ fissi.
...
## Hyperparameters (Non servono, ma devo comunque definirli)
...
$(\Lambda,\gamma_1,\dots,\gamma_d)$ prior elicitation (non serve per il momento, lascio solo perché devo comunque definire gli iperparametri)
...
gamma_mean = rep( mygamma , d)
lambda_mean = mylambda
Kmax   = 20
pk = rep(0,Kmax)
for(k in 1:Kmax){
  #cat("\n k = ",k,"\n")
...
cat("\n sum(pk) = ",sum(pk),"\n")
par(mar = c(2,2,2,1), bty = "l")
plot(pk, type = "h", pch = 16, lty = 1, lwd = 2, xaxt = "n", xlab = "K", main = "Theoretical K")
...
- Caso 1: $\Lambda = 25$, $\gamma_1 = \dots = \gamma_d = 0.01$ -> teoria e MCMC coincidono
- Caso 2: $\Lambda = 25$, $\gamma_1 = \dots = \gamma_d = 1/200$ -> teoria e MCMC coincidono ma servono più iterazioni rispetto a prima. Inoltre, $K$ si schiaccia un po' di più verso i valori piccoli
- Caso 3: $\Lambda = 5$, $\gamma_1 = \dots = \gamma_d = 0.01$ -> Tutto bene, valori teorici e MCMC coincidono. 
- Caso 4: $\Lambda = 5$, $\gamma_1 = \dots = \gamma_d = 1$ -> Tutto bene, valori teorici e MCMC coincidono ma dopo tante iterazioni (100k). In realtà facendo meno iterazioni sembra vada tutto abbastanza bene, solo non sono precisissimi. In questo caso, tutte le componenti sono attive, quindi $K$ e $M$ coincidono.
...
B = 10000
M_mar_prior = rpois(n=B, lambda = mylambda ) + 1


xmax = 20
par(mar = c(2,2,2,1), bty = "l")
plot(pk_MCMC, type = "h",  ylim = c(0,0.5), xlim = c(0, xmax), col = "black", 
     xlab = " ", ylab = " ", main = "Prior comparison", xaxt = "n")
mtext(text = as.character(1:xmax), side = 1, at = 1:xmax, cex = 0.75)
...
legend("topright", c("K - Exact","K - MCMC","M - MC", "M - MCMC"), 
       col = c("lightblue4","black","red","orange"), pch = 16)
...
- Caso 1: $\Lambda = 25$, $\gamma_1 = \dots = \gamma_d = 0.01$ -> Ha senso, $M$ coincide con curva teorica, $K$ è minore di $M$
- Caso 2: $\Lambda = 25$, $\gamma_1 = \dots = \gamma_d = 1/200$ -> Ha senso, $M$ coincide con curva teorica, $K$ è minore di $M$ e anche minore rispetto a prima
- Caso 3: $\Lambda = 5$, $\gamma_1 = \dots = \gamma_d = 1$ -> Non mixxa
- Caso 4: $\Lambda = 5$, $\gamma_1 = \dots = \gamma_d = 0.01$ -> con questi parametri mi aspetto che $K$ e $M$ coincidano. $M$ nel MCMC sovrastima $M$ teorico ma non di troppo
- Caso 5: $\Lambda = 10$, $\gamma_1 = \dots = \gamma_d = 1e-5$ -> Non mixxa
...
points(pm_MCMC, type = "h", col = "orange", xlab = "K")
points(table(M_mar_prior)/B, type = "h", col = "red", xlab = "M")
legend("topright", c("K - MCMC","M - MC", "M - MCMC"), 
       col = c("black","red","orange"), pch = 16)
```
...
$P_0$
```{r}
Res_range = range( data )
R = Res_range[2] - Res_range[1]
mu0 = mean(data) # should be 0
k0  = 1/R^2
nu0 = 10
sigma0 = 10#*(100*R/1)
c("mu0"=mu0,"k0"=k0,"nu0"=nu0,"sigma0"=sigma0)
...
points(x = na.omit(as.numeric(data)), 
       y = rep(0, length(na.omit(as.numeric(data)))), 
       ylim = c(0,0.1),
       pch = 16)
...
- Caso 1: $\texttt{Exp_Lambda} = 25$, $\texttt{Var_Lambda} = 2$, $\texttt{gamma_guess} = 0.01$ -> come quello che ottengo con il condizionale
...
option = set_options_marginal(
             ...
             "UpdateU" = T, "UpdateGamma" = F, "UpdateTau" = T, "UpdateLambda" = F, 
...
grid_lambda = seq(1e-8,50,length.out = 10000)
B = 10000
...
points(table(M_mar_prior)/B, type = "h", col = "red", xlab = "M")
legend("topright", c("K - MCMC","Lambda","M - MC"), 
