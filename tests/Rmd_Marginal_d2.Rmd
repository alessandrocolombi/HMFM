...
**Attenzione**: i codici per fare i plot non funzionano che le cardinalità dei gruppi sono diverse. Ho sistemato in _RmdMarginal d high_
...
Cerco di fare la stessa cosa con il numero di cluster condivisi, $S$

1. _c'è qualcosa che non va, non sempre mi somma ad 1_
2. Ho capito perché è cosi lenta: per calcolare S, ogni volta devo fare una somma da ss a n, numero totale di dati. Quindi la complessità aumenta con il numero di dati, cosa che invece non succede quando si vuole calcolare $K$
```{r}
# prob_s = rep(0,min(n_j)+1)
# threshold = 0.995
# for(ss in 0:length(prob_s) ){
#     prob_s[ss] = p_shared_prior(s = ss, n_j = n_j, 
#                               gamma = c(0.5,0.5),
#                               prior = "Poisson", lambda = 20,
#                               Max_iter = 10)
#     # prob_s[ss] = p_shared_prior(s = ss, n_j = n_j, 
#   #                             gamma = c(optK$par[1],optK$par[1]),
#   #                             prior = "Poisson", lambda = optK$par[2],
#   #                             Max_iter = 1000)
#   #if(sum(prob_s)>=threshold)
#    # break
# }
# 
# plot(which(prob_s>0), prob_s[prob_s>0], 
#      type = 'b', pch = 16, lty = 1, lwd = 3,
#      main = "Prior number of shared clusters", xlab = "S", ylab = "prob")
# 
# sum(prob_s)
```
...
* Qua ho visto un vero e proprio trade-off tra clustering e density estimation
  - tramite il parametro $correction$ decido se stringere o no la marginale a priori. Più lo aumento, più la stringo
  - anche aumentando $\bar{V}_\mu$ la stringo ma così facendo modifico solo $k_0$ e non cambia niente
  - Aumentando $correction$, quindi stringendo molto, la density estimation migliora sensibilmente ma il clustering peggiora: il numero di cluster aumenta molto, non ritrovo la partizione giusta nemmeno con VI 
  - Diminuendo $correction$ succede l'opposto: clustering perfetto ma la densità stimata sembra più bassa (ora che ci penso, chi mi dice che quegli istogrammi abbiamo l'altezza giusta??)
  - Dopo vari tentativi, $correction = 20$ sembra un buon trade-off
...
Anche in questo caso, bisogna giocare con il parametro correction
...
Anche qui, bisogna giocare con il parametro correction


# Test con cluster più vicini

Rispetto alla sezione precedente, considero meno dati e cluster più vicini. 
...
* d = 2
* $n_1 = 150$, $n_2 = 150$
* $\mu_1 = -1$, $\mu_2 = 0$, $\mu_3 = 2$
* $\sigma_1 = \sqrt{1/8}$ $ \sigma_2 = \sqrt{1/4}$ $\sigma_3 = \sqrt{1/8}$
* $\pi_{11} = \pi_{12} = 1/4$ $\pi_{13} = 1/2$
* $\pi_{21} = 3/5$ $\pi_{22} = 1/5 $$ \pi_{23} = 1/5$
...
mu = c(-3,0, 2)                     # vectors of means
sd = c(sqrt(1/8),sqrt(1/8),sqrt(1/16)) # vector of sd
n_j= c(150,150)                        # set cardinality of the groups
mix_probs = matrix(c(1/3,1/3,1/3,   # mixture weigths
                     0.30,0.30,0.4), nrow = d, ncol = K, byrow = T)
seed = 20051131
...
l_grid = 200
grid = seq(-3,4,length.out = l_grid)
...
for(j in 1:d){
  colors_level = as.factor(real_partition[idx_start:idx_end])
  levels(colors_level1) = mycol_cluster
  
  print(table(real_partition[idx_start:idx_end]))
  
  dens_lev = density(data[j,1:n_j[j]])
  
  plot(dens_lev, lwd = 3, col = mycol[3], main = paste0("level = ",j), xlab = "values", ylab = "denisty")
  polygon(dens_lev, col = ACutils::t_col(mycol[1], percent = 70))
  points(x=data[j,1:n_j[j]], y = rep(0,n_j[j]), pch = 16, col = colors_level)
...
# Calcolo anche la marginale trovata con i valori di Empirical Bayes e la plotto sopra
P0_hyparam = empirical_bayes_normalinvgamma(data = data, varmu = 1, varsig2 = 10, correction = 3)
mu0 = P0_hyparam$mu0;k0= P0_hyparam$k0;nu0=P0_hyparam$nu0;sigma0=P0_hyparam$sigma0

l_grid = 200
grid = seq(-3,5,length.out = l_grid)
scale = sqrt( k0/(k0 + 1) * sigma0 )
mean_marginal = mu0
var_marginal  = nu0/(nu0-2) * scale^2
...
# Compute Rand Index
fossil::rand.index(binder_sara$cl, real_partition)
fossil::rand.index(VI_sara$cl, real_partition)
```


```{r}
l_grid = 200
grid = seq(-4,6,length.out = l_grid)
...
# Test difficile Raf e Cremaschi
...
# Calcolo anche la marginale trovata con i valori di Empirical Bayes e la plotto sopra
P0_hyparam = empirical_bayes_normalinvgamma(data = data, barmu = mean(data), barsig2 = 5, correction = 3 )
mu0 = P0_hyparam$mu0;k0= P0_hyparam$k0;nu0=P0_hyparam$nu0;sigma0=P0_hyparam$sigma0
...
Expected_prior(n_j = n_j, gamma = c(optK$par[1],optK$par[1]), 
                      type = "distinct", prior = "Poisson", 
                      lambda = optK$par[2])
```
