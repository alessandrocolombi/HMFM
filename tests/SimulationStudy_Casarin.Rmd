...
P0_hyparam = empirical_bayes_normalinvgamma(data = data, varmu = 100, varsig2 = varsig2, correction = 50)
mu0 = P0_hyparam$mu0
k0= P0_hyparam$k0
nu0=P0_hyparam$nu0
sigma0=P0_hyparam$sigma0
...
varmu_grid = c(1,5,10,50,100,vardata,vardata^2)
N_varmu = length(varmu_grid)
nomi_varmu_grid = c("Vmu=1","Vmu=5","Vmu=10","Vmu=50","Vmu=100",
                    "Vmu=Vdata", "Vmu=Vdata^2")
...
```{r}
Var_pred_in_cluster = matrix(0,nrow = N_varsig, ncol = d)
...
```{r}
Var_pred_in_cluster = matrix(0,nrow = N_correction, ncol = d)
...
varmu_grid = c(5,10,50,100,vardata,vardata^2)
...
nomi_varmu_grid = c("Vmu=5","Vmu=10","Vmu=50","Vmu=100",
                    "Vmu=Vdata", "Vmu=Vdata^2")
...
names(RI_Bin) <- names(RI_VI) <- names(L1) <- names(Kest) <- names(Kest_var) <- names(MC_Kest) <- names(CoClustering_error) <- paste0("Lambda=", Lambda_grid)
