---
title: "Simulation Study - Casarin"
...
varmu_grid = c(1,5,10,vardata,50,100,vardata^2)
N_varmu = length(varmu_grid)
nomi_varmu_grid = c("Vmu=1","Vmu=5","Vmu=10","Vmu=Vdata","Vmu=50","Vmu=100","Vmu=Vdata^2")
...
Rispetto alla sezione precedente, non lascio $\Lambda$ fisso ma aleatorio. Parametrizzo $\Lambda$ come media e varianza. Metto la media pari ai valori fissati prima e varianza 10 perché voglio vedere se si muove da quel valore
...
names(Lambda_mean) <- names(Lambda_mean) <- paste0("MeanLambda=", mean_Lambda_grid)
...
rownames(gamma_means) <- paste0("MeanLambda=", mean_Lambda_grid)
colnames(gamma_means) <- paste0("Lev=", 1:d)
...
Peggiora sensibilmente al crescere di $b_\gamma$
