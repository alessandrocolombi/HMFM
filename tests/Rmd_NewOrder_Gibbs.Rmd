...
Ho notato che se $\nu$ = 1 la catena non mixxa per niente (e questo già lo sapevamo).
Però per $\nu<1$ i risultati sono stabili, ho provato $\nu = 1/10$, $\nu = 0.5$, $\nu = 1/100$ e arrivano circa alla stessa soluzione.
...
option = set_options(
             "nu" = 1/100, "Mstar0" = 2, "Lambda0" = 3, "mu0" = 0,"sigma0"= 1, "gamma0" = 1,
...
# Questa parte va aggiornata usando il nuovo pacchetto di sara wade!
...
- $(n_1,\dots,n_{10}) = (100,\dots,100)$.
- parto da partizione vera.
- $\Lambda$ è fissato ad un valore piccolo e non viene aggiornato.


Con $\Lambda = 0.5$, K si assesta subito su circa 15 (non esplode) e la partizione finale (con VI) è ancora su circa 13 cluster. Quella con la binder loss invece è bruttissima, con un sacco di singoletti.

Con $\Lambda = 0.1$, K si assesta subito su circa 12 (non esplode) e la partizione finale, con VI, è perfetta!
Quella con la binder non va bene, ci sono ancora tantissimi singoletti (138 cluster stimati). Anche se non si parte della partizione vera ma da quella vuota, si arriva allo stesso risultato. 
...
option = set_options(
             "nu" = 1/10, "Mstar0" = 2, "Lambda0" = 0.1, "mu0" = 0,"sigma0"= 1, "gamma0" = 1,
             "Adapt_MH_hyp1"= 0.7,"Adapt_MH_hyp2"= 0.234, "Adapt_MH_power_lim"=10, "Adapt_MH_var0"=1,
             "k0"= 1/10, "nu0"=10, "alpha_gamma"=1,
             "beta_gamma"=1, "alpha_lambda"=1, "beta_lambda"=1,
             "UpdateU" = T, "UpdateM" = T, "UpdateGamma" = T, "UpdateS" = T,
             "UpdateTau" = T, "UpdateLambda" = F, "partition" = NULL
        )
...
#K
plot(GDFMM$K[1:3000], type = 'l', main = "K")
hist(GDFMM$K, main = "K")

#Mstar
plot(GDFMM$Mstar[1:3000], type = 'l', main = "Mstar")
