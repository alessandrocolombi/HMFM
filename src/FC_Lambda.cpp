...
void FC_Lambda::update(GS_data& gs_data, const sample::GSL_RNG& gs_engine){
    ...
    // UPDATE ROUTINE
    double a2_star = static_cast<double>( (k-1) ) + a2;
    ...
    double p0 = (a2_star)/( (a2_star-1.0) +
                            (double)k * (b2+1.0)*exp(log_sum)
                           );
    // Select, via extraction from a uniform, which distribution sample from
    bool select_p0 = binary_decision(p0, gs_engine);

    if(select_p0)
        gs_data.lambda = Gamma(gs_engine, a2_star + 1.0, 1.0 /(b2 + 1.0 - exp(-log_sum)) );
    else
        gs_data.lambda = Gamma(gs_engine, a2_star, 1.0 /(b2 + 1.0 - exp(-log_sum)) );
}
